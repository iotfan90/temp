...
class OmniturePy(object):
    ...
    def __get_header(self):
        ...
        sha_object = hashlib.sha1(nonce + created_date + self.shared_secret)
    ...
    def run_omtr_queue_and_wait_request(self, method, request_data,max_polls=20,
        ...
        # We exit the while loop only when the report is done or the report has
        # failed and passed the max retries.
        if status == 'failed':
        ...
        return self.run_omtr_immediate_request('Report.GetReport',
                                               {'reportID': report_id})

    def get_count_from_report(self, report_suite_id, metric, element=None,
                              selected_element_list=None,
                              date_from=YESTERDAY_DATE, date_to=YESTERDAY_DATE,
        ...
        if element is None:
            ...
            elements = [{
                'id': element,
                'selected': selected_element_list
            }]

        # Invoke Omniture API with properly formed JSON Request
        response = self.run_omtr_queue_and_wait_request(
            request_type, {
                'reportDescription': {
